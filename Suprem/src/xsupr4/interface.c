...
//    tmp->Title = tmp->XUnitText = tmp->YUnitText = NULL;
    tmp->Title = strdup("xxxxxxxx");
    tmp->XUnitText = strdup("xxxxxxxx");
    tmp->YUnitText = strdup("xxxxxxxx");
/*** Changed by Kaushik ends ***/
    return(tmp);
}
...
/*** Changed by Kaushik to resolve segmentation violation during plotting ***/
	/*newwin = NULL;*/
	if ( newwin == NULL)
		newwin = make_datawin();
	xgSetBounds((double)0.0, (double)400.0, (double)0.0, (double)400.0);
	if ( app_context == NULL )
		make_graph(newwin);
/*** Changed by Kaushik ends ***/
    toolkitinit = FALSE;
